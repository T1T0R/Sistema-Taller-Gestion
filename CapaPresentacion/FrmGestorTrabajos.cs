    ...
    public partial class FrmGestorTrabajos : Form
    {
        ...
        private void FrmGestorTrabajos_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                // Configurar las columnas para que se vean mejor
                dgvGestorTrabajos.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.DisplayedCells;
                dgvGestorTrabajos.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCells;

                // Ajustar tamaño del DataGridView para evitar que ocupe demasiado espacio
                dgvGestorTrabajos.MaximumSize = new Size(this.Width - 8, this.Height - 80);

                // Habilitar el desplazamiento si hay muchas columnas
                dgvGestorTrabajos.ScrollBars = ScrollBars.Both;

                // Ajustar el texto dentro de las celdas para evitar que se corte
                dgvGestorTrabajos.DefaultCellStyle.WrapMode = DataGridViewTriState.True;
                ...
                dgvGestorTrabajos.AlternatingRowsDefaultCellStyle.BackColor = Color.LightGray;
                dgvGestorTrabajos.DefaultCellStyle.SelectionBackColor = Color.DodgerBlue;
                dgvGestorTrabajos.DefaultCellStyle.SelectionForeColor = Color.White;

                // Evitar que las filas ocupen demasiado espacio
                dgvGestorTrabajos.RowHeadersVisible = false;

                // Asegurar que el DataGridView se ajusta correctamente dentro del formulario
                dgvGestorTrabajos.Anchor = AnchorStyles.Top | AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Bottom;
        ...
        private void cmbModificar_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cmbModificar.SelectedIndex > -1)
                {
                    ...
                    else if (seleccion == "Equipo" || seleccion == "Categoria" || seleccion == "FallaDeclarada" || seleccion == "Modelo" ||
                             seleccion == "NumeroSerie" || seleccion == "Detalles" || seleccion == "Control" ||
                             seleccion == "Cable" || seleccion == "Base")
                    {
                        resultados = trabajos.ObtenerEstadoEquipos();
                        label11.Text = "Ingrese el Codigo del Equipo:";
                    }
                    else if (seleccion == "Presupuesto" || seleccion == "PrecioArreglo")
                    {
